...
void GameView::render() {
    if (renderer) {
        ...
        for (int i = 0; i < game.getTowers().size(); ++i) {
            ...
            SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255); // Black color for towers
        ...
        for (int i = 0; i < game.getTowers().size(); ++i) {
            ...
            std::stack<Disk> tempStack = tower.getDisks();

            while (!tempStack.empty()) {
                const Disk& disk = tempStack.top();
                SDL_Rect diskRect = calculateDiskRect(disk, i); // Pass tower index for positioning
                SDL_SetRenderDrawColor(renderer, disk.getColor().r, disk.getColor().g, disk.getColor().b, 255);
...
SDL_Rect GameView::calculateTowerRect(int towerIndex) {
    ...
    int spacing = windowWidth / 4; // Dividing window into 4 parts for 3 towers
